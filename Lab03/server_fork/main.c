...
int main(){
	...
	bind (listenfd,(struct socjaddr *) &servaddr, sizeof(servaddr));
	//listening
	...
	while (true){
		...
		if (connfd<0){
			printf("Client %d disconnect. ",cli_nr);
			return false;
		}
		else{
			cli_nr++;
		...
		if ((childpid=fork())==0){
			printf("Child pid creat for dealing with client %d  request.\n",cli_nr);
			printf("Recieved client %d request...\n",cli_nr);
			close(listenfd);
